...
public class TestCase {
	public static void main(String[] args) {
		...
		try {
			...
			int freq;
			System.out.println(
					"-- checking s4.B193362.Frequencer.frequency() return -1, when TARGET is not set or TARGET's length is zero");
			myObject = new s4.B193362.Frequencer();
			myObject.setSpace("Hi Ho Hi Ho".getBytes());
			freq = myObject.frequency();
		...
		try {
			...
			int freq;
			System.out.println(
					"-- checking s4.B193362.Frequencer.frequency() return 0, when SPACE is not set or Space's length is zero");
			myObject = new s4.B193362.Frequencer();
			myObject.setTarget("H".getBytes());
			freq = myObject.frequency();
		...
		try {
			...
			System.out.println("|  subByte 2-3 of \"aaHaa\" in \"Hi Ho Hi Ho\" appears " + freq + " times. ");
			if (4 == freq) {
				System.out.println("-- OK --------------------------------\n");
			} else {
				System.out.println("-- WRONG --------------------------------\n");
		...
		try {
			...
			System.out.println("|  > 0    " + value + " == 2.0");
			ok = ok && Double.compare(value, 2.0) == 0;
			myObject.setTarget("01".getBytes());
			value = myObject.estimation();
			System.out.println("|  > 01   " + value + " == 3.0");
			ok = ok && Double.compare(value, 3.0) == 0;
			myObject.setTarget("0123".getBytes());
			value = myObject.estimation();
			System.out.println("|  > 0123 " + value + " == 3.0");
			ok = ok && Double.compare(value, 3.0) == 0;
			myObject.setTarget("00".getBytes());
			value = myObject.estimation();
			System.out.println("|  > 00   " + value + " == 4.0");
		...
		try {
			...
			double value;
			System.out.println(
					"-- checking s4.B193362.InformationEstimator.estimation() returns 0.0 when the target is not set or Target's length is zero");
			myObject = new s4.B193362.InformationEstimator();
			myObject.setSpace("3210321001230123".getBytes());
			value = myObject.estimation();
		...
		try {
			...
			double value;
			System.out.println(
					"-- checking s4.B193362.InformationEstimator.estimation() returns Double.MAX_VALUE, when the true value is infinite, or space is not set");
			myObject = new s4.B193362.InformationEstimator();
			myObject.setTarget("0".getBytes());
			value = myObject.estimation();
			System.out.println("|  > " + value);
